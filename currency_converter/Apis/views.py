...
from .serializers import CurrencyConversionSerializer, CurrencyConversionOutputSerializer, CurrencyItemSerializer, CurrencyExchangeRateInputSerializer, CurrencyExchangeRateOutputSerializer
...
# will have to refactor since the code is kinda repeated
# or make amount not important, if amount isnt added in the currency conversion serializer request it should default to one and just give the plain exchange rate
class CurrencyExchangeRateView(APIView):
    """
    API View to fetch the exchange rate between two currencies (crypto or fiat).
    """
    @extend_schema(
        request=CurrencyExchangeRateInputSerializer,
        responses={200: CurrencyExchangeRateOutputSerializer},
        summary="Fetch the exchange rate between two currencies (crypto or fiat).",
        description="Fetches the latest exchange rate or historical exchange rate based on the provided date."
    )
    def post(self, request, *args, **kwargs):
        input_serializer = CurrencyExchangeRateInputSerializer(data=request.data)
        ...
        output_serializer = CurrencyExchangeRateOutputSerializer(output_data_dict)
